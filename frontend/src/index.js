...
class VideoCallApp {
    ...
    getWebSocketUrl() {
        ...
        const base = isLocalDev ? `${wsProtocol}://${hostname}:3002` : `${wsProtocol}://${host}`;
        const path = "/ws";
        return `${base}${path}?callId=${encodeURIComponent(this.callId)}&peerId=${encodeURIComponent(this.peerId)}&token=${encodeURIComponent(this.joinToken)}`;
    }
    ...
    async joinCall() {
        ...
        if (Array.isArray(data.turnCredentials?.iceServers) && data.turnCredentials.iceServers.length > 0) {
