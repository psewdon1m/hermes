...
const {
  ...
  INACTIVE_TTL_SECONDS = '3600', // 1h без активности
} = process.env;
...
async function addPeer(callId, peerId) {
  ...
  await redis.hset(`call:${callId}`, { status: 'active', updatedAt: String(Date.now()) });
  await redis.persist(`call:${callId}`); // снимаем TTL — активный звонок
}

async function removePeer(callId, peerId) {
  await redis.srem(`call:${callId}:peers`, peerId);
  const sz = await redis.scard(`call:${callId}:peers`);
  if (sz === 0) {
    // Никого не осталось — откатываем в pending и ставим часовой TTL
    await redis.hset(`call:${callId}`, { status: 'pending', updatedAt: String(Date.now()) });
...
wss.on('connection', async (ws, req) => {
  try {
    ...
    // Проверяем, жива ли комната (не истёк час простоя)
    ...
    ws.on('message', async (data) => {
      ...
      for (const [otherWs, meta] of sockets) {
        if (meta.callId === callId && meta.peerId === to && otherWs.readyState === 1) {
    ...
    ws.on('close', async () => {
      ...
      for (const [otherWs, o] of sockets) {
        if (o.callId === meta.callId) send(otherWs, { type: 'peer-left', peerId: meta.peerId, left: leftSize });
      }
    });
